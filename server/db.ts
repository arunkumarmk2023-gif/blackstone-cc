...
// Gallery queries
export async function createGalleryImage(image: InsertGallery) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db.insert(gallery).values(image);
}
...
export async function getGalleryImagesByCategory(category: string) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(gallery).where(eq(gallery.category, category as any)).orderBy(desc(gallery.displayOrder));
}

export async function updateGalleryImage(id: number, updates: Partial<InsertGallery>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db.update(gallery).set(updates).where(eq(gallery.id, id));
}
...
export async function getFeaturedGalleryImages(limit: number = 6) {
  const db = await getDb();
  if (!db) return [];
...
// Joiner functions
export async function createJoiner(joiner: InsertJoiner) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db.insert(joiners).values(joiner);
}

export async function getJoiners() {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(joiners).orderBy(desc(joiners.createdAt));
}

export async function getJoinerById(id: number) {
  const db = await getDb();
  if (!db) return null;
  const result = await db.select().from(joiners).where(eq(joiners.id, id));
  return result[0] || null;
}

export async function getJoinersByStatus(status: string) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(joiners).where(eq(joiners.status, status as any)).orderBy(desc(joiners.createdAt));
}

export async function updateJoinerStatus(id: number, status: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db.update(joiners).set({ status: status as any }).where(eq(joiners.id, id));
}

export async function deleteJoiner(id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
